...
const ChatList = () => {
      ...
      setChatUsers((prevChatUsers) => {
        const existingChat = prevChatUsers.find((chat) =>
          chat.participants.some(
            (user) => user._id === message.senderId || user._id === message.receiverId
          )
        );
  ...
  useEffect(() => {
    axios
      .get("http://localhost:8000/api/v1/groups/my-groups", {
        headers: { Authorization: `Bearer ${token}` },
      })
      .then((response) => {
  ...
  const renderChats = () => {
    if (activeTab === "chats") {
      ...
      // inside your renderChats() in ChatList.jsx, for groups tab:
  ...
  return (
    <div className="chat-list">
      ...
      <div className="chat-content">{renderChats()}</div>
